...
{
    public partial class AddOrganizationNameToMyCourseList : Migration
    {
...
RETURN
(
SELECT c.Id, sbi.Name AS CourseName, ct.ActiveFrom, ct.ActiveTo, ttf.Value AS TimeFrom, ttt.Value AS TimeTo, uio.UserId, csbi.Name AS ClassRoom, bsbi.Name AS  BranchName,
ct.Monday, ct.Thursday, ct.Wednesday, ct.Tuesday, ct.Friday, ct.Saturday, ct.Sunday,osbi.Name AS OrganizationName
...
ct.Monday,ct.Thursday,ct.Wednesday,ct.Tuesday, ct.Friday, ct.Saturday, ct.Sunday,osbi.Name AS OrganizationName
  ...
  JOIN Edu_Organization AS eo ON eb.OrganizationId = eo.Id AND eo.IsDeleted = 0
  JOIN Shared_BasicInformation AS  osbi ON osbi.Id = eo.BasicInformationId AND osbi.IsDeleted = 0
  WHERE c.IsDeleted = 0  AND ct.ActiveTo >= SYSDATETIME() AND lcs.UserId =@userId
)");
