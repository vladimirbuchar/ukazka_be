...
{
    public partial class GetAllStudentTestResultCourseId : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"ALTER FUNCTION [dbo].[GetAllStudentTestResult](@courseId uniqueidentifier )
RETURNS TABLE  AS
RETURN 
( 
SELECT sts.Id, p.FirstName, p.SecondName, p.LastName, bi.Name, u.UserEmail, sts.Finish,sts.Score,sts.TestCompleted
  ...
  WHERE sts.CourseId = @courseId AND  sts.IsDeleted = 0 AND eor.SystemIdentificator ='STUDENT' 
)");
        }
